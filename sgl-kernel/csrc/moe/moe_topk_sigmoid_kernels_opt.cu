...
__launch_bounds__(TPB) __global__ void moeSigmoidOpt(
...
template <int TPB>
__launch_bounds__(TPB) __global__ void moeTopKOpt(
...
// ====================== TopK sigmoid things (OPTIMIZED) ===============================

/*
  OPTIMIZED Top-K gating sigmoid kernel.

  Key optimization: Exploit the monotonicity and element-wise independence of sigmoid.

  For sigmoid: sigmoid(a) > sigmoid(b) iff a > b (monotonicity)
  Unlike softmax, sigmoid(x_i) only depends on x_i (independence)

  This allows us to:
  1. Perform TopK selection directly on raw logits (no need to compute sigmoid first)
  2. Only compute sigmoid for the selected TopK elements

  This reduces exp() computations from num_experts to just topk!

  Note: When correction_bias is provided, we must compute sigmoid first because
  the ranking is based on sigmoid(x) + bias, which breaks monotonicity.
...
__launch_bounds__(WARPS_PER_CTA* WARP_SIZE) __global__ void topkGatingSigmoidOpt(
    const T* input,
    const bool* finished,
    float* output,
    const int num_rows,
...
#pragma unroll
  ...
  // OPTIMIZATION: Different paths based on whether correction_bias is provided
  // - Without bias: Work on raw logits (skip sigmoid computation until TopK is found)
  // - With bias: Must compute sigmoid first (bias breaks monotonicity)

  float row_chunk[VPT];

  if (correction_bias != nullptr) {
    // With correction_bias: compute sigmoid + bias for comparison (original path)
#pragma unroll
    ...
    // OPTIMIZED PATH: Keep raw logits for comparison (no sigmoid yet!)
#pragma unroll
    for (int ii = 0; ii < VPT; ++ii) {
  ...
  // Now, row_chunk contains either:
  // - raw logits (if no correction_bias) - for optimized path
  // - sigmoid + bias (if correction_bias) - for original path
...
#pragma unroll
    ...
    if (thread_group_idx == 0) {
      ...
      float output_val;
      if (correction_bias != nullptr) {
        // max_val is sigmoid + bias, subtract bias to get sigmoid
        output_val = max_val - correction_bias[expert];
      } else {
        // OPTIMIZATION: max_val is raw logit, compute sigmoid now (only for TopK elements!)
        output_val = 1.0f / (1.0f + expf(-max_val));
      }

      output[idx] = output_val;
      indices[idx] = should_process_row ? (expert - start_expert) : NUM_EXPERTS;
      row_sum_for_renormalize += output_val;
    }
    ...
    if (k_idx + 1 < k) {
      ...
      if (thread_group_idx == thread_to_clear_in_group) {
        ...
        // Use different sentinel values:
        // - For raw logits: use -FLT_MAX (very negative logit)
        // - For sigmoid + bias: use -10000.f (works since sigmoid is in [0,1])
        row_chunk[ldg_group_for_expert * ELTS_PER_LDG + offset_for_expert] =
            (correction_bias != nullptr) ? -10000.f : -FLT_MAX;
      }
    }
  }
...
void topkGatingSigmoidLauncherHelperOpt(
    const T* input,
    const bool* finished,
    float* output,
    int* indices,
  ...
  topkGatingSigmoidOpt<T, VPT, EXPERTS, WARPS_PER_TB, BYTES_PER_LDG><<<num_blocks, block_dim, 0, stream>>>(
      input, finished, output, num_rows, indices, k, start_expert, end_expert, renormalize, correction_bias);
}

#define LAUNCH_SIGMOID_OPT(TYPE, NUM_EXPERTS, WARPS_PER_TB)             \
  topkGatingSigmoidLauncherHelperOpt<TYPE, NUM_EXPERTS, WARPS_PER_TB>( \
      gating_output,                                                \
      nullptr,                                                      \
      topk_weights,                                                 \
      topk_indices,                                                 \
...
template <typename T>
void topkGatingSigmoidKernelLauncherOpt(
  ...
  switch (num_experts) {
    case 1:
      LAUNCH_SIGMOID_OPT(T, 1, WARPS_PER_TB);
      break;
    case 2:
      LAUNCH_SIGMOID_OPT(T, 2, WARPS_PER_TB);
      break;
    case 4:
      LAUNCH_SIGMOID_OPT(T, 4, WARPS_PER_TB);
      break;
    case 8:
      LAUNCH_SIGMOID_OPT(T, 8, WARPS_PER_TB);
      break;
    case 16:
      LAUNCH_SIGMOID_OPT(T, 16, WARPS_PER_TB);
      break;
    case 32:
      LAUNCH_SIGMOID_OPT(T, 32, WARPS_PER_TB);
      break;
    case 64:
      LAUNCH_SIGMOID_OPT(T, 64, WARPS_PER_TB);
      break;
    case 128:
      LAUNCH_SIGMOID_OPT(T, 128, WARPS_PER_TB);
      break;
    case 256:
      LAUNCH_SIGMOID_OPT(T, 256, WARPS_PER_TB);
      break;
    default: {
      TORCH_CHECK(
      ...
      moeTopKOpt<TPB><<<num_tokens, TPB, 0, stream>>>(
          sigmoid_workspace,
          nullptr,
          topk_weights,
          topk_indices,
...
void topk_sigmoid_opt(
  ...
  if (dtype == at::ScalarType::Float) {
    topkGatingSigmoidKernelLauncherOpt<float>(
    ...
    topkGatingSigmoidKernelLauncherOpt<__half>(
    ...
    topkGatingSigmoidKernelLauncherOpt<__nv_bfloat16>(
